/******************************************************************************************
*                                                                                         *
*	** THIS FILE HAS BEEN MODIFIED FROM ITS ORIGINAL VERSION - CMB	**					  *															
*                                                                                         *
*                                                                                         *
...
Surface Surface::FromFile( const std::wstring & name )
{
	...
	std::unique_ptr<unsigned int[]> localRGBAbuffer = nullptr;

	{
		...
		localRGBAbuffer = std::make_unique<unsigned int[]>(width * height);
		...
		for( unsigned int y = 0; y < height; y++ )
		{
			for( unsigned int x = 0; x < width; x++ )
			{
				...
				unsigned int colorValue = c.GetValue();
				pBuffer[y * pitch + x] = colorValue;				
				unsigned int rgbaValue = (((colorValue & 0xF0000000) >> 24) | colorValue << 8);		
				localRGBAbuffer[y * pitch + x] = rgbaValue;

			}
		}
	}

	return Surface( width,height,pitch,std::move( pBuffer ), std::move(localRGBAbuffer));
}
...
void Surface::Save( const std::wstring & filename ) const
{
	...
	Gdiplus::Bitmap bitmap( width,height,pitch * sizeof( Color ),PixelFormat32bppARGB,(BYTE*)pBuffer.get() );   // pBuffer.get -- restore for unique_ptr
...
void Surface::Copy( const Surface & src )
{
	...
	if( pitch == src.pitch )
	{
		memcpy(pBuffer.get(), src.pBuffer.get(), pitch * height * sizeof(Color));  // unique_ptr
