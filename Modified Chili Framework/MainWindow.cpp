	...
	hInst( hInst )
{
	...
	RECT wr;
	wr.left = 350;
	wr.right = Software3D::D3DInterface::ScreenWidth + wr.left;
	wr.top = 100;
	wr.bottom = Software3D::D3DInterface::ScreenHeight + wr.top;
	AdjustWindowRect( &wr,WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU,FALSE );
	hWnd = CreateWindow( wndClassName,L"Software3D",
...
LRESULT MainWindow::HandleMsg( HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam )
{
	...
	if (imguiIO.WantCaptureKeyboard && imguiIO.WantCaptureMouse) return true;
	...
	case WM_MOUSEMOVE:
	{
		int x = LOWORD( lParam );
		int y = HIWORD( lParam );
		if( x > 0 && x < Software3D::D3DInterface::ScreenWidth && y > 0 && y < Software3D::D3DInterface::ScreenHeight )
		{
		...
		else
		{
			...
			{
				x = std::max( 0,x );
				x = std::min( int( Software3D::D3DInterface::ScreenWidth ) - 1,x );
				y = std::max( 0,y );
				y = std::min( int( Software3D::D3DInterface::ScreenHeight ) - 1,y );
				mouse.OnMouseMove( x,y );
			}
			else
			{
